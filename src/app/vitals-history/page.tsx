...
function CTooltip({ active, payload, label }: CTooltipProps) {
  ...
  return (
    ...
    >
      <p className="font-mono mb-1.5" style={{ color: MUTED }}>
        {label}
      </p>
      {payload.map((e) => (
    ...
    <AppLayout>
      <div className="h-full overflow-y-auto scrollbar-thin">
        <div className="px-6 py-5 max-w-screen-2xl mx-auto space-y-5">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs mt-0.5" style={{ color: MUTED }}>
                24-hour historical vitals · per patient
              </p>
            </div>
          ...
          {/* Patient summary bar */}
          <div
            className="flex items-center gap-5 p-4 rounded-2xl border"
            style={{ backgroundColor: CARD, borderColor: BORDER }}
          >
            <div
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs mt-0.5" style={{ color: MUTED }}>
                {selectedPatient.age}y · {selectedPatient.gender} · {selectedPatient.diagnosis} ·{' '}
                {selectedPatient.bedId.toUpperCase()}
              </p>
            </div>
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              >
                Attending:{' '}
                <span className="text-white font-medium">{selectedPatient.attendingPhysician}</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
            {VITAL_CONFIGS.map((v) => {
              const val = selectedPatient.vitals[
                v.key as keyof typeof selectedPatient.vitals
              ] as number;
              ...
              return (
                ...
                >
                  ...
                  <p
                    ...
                    style={{ color: isAlert ? v.color : 'white' }}
                  >
                    {val}
                    {v.key === 'temp' ? '' : ''}
                  </p>
                  ...
                  <p className="text-[9px] font-mono mt-0.5" style={{ color: MUTED }}>
                    {v.unit} · norm {v.refLow}–{v.refHigh}
                  </p>
                </div>
              );
            })}
              ...
              >
                ...
                <ResponsiveContainer width="100%" height={150}>
                  <AreaChart data={trendData} margin={{ top: 5, right: 5, left: -30, bottom: 0 }}>
                    ...
                    <Area
                      type="monotone"
                      dataKey={v.key as string}
                      name={`${v.label} (${v.unit})`}
                      stroke={v.color}
                      strokeWidth={2}
                      fill={`url(#grad-${v.key})`}
                      dot={false}
                      activeDot={{ r: 3, fill: v.color, strokeWidth: 0 }}
                    />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
