...
function CustomTooltip({ active, payload, label }: CustomTooltipProps) {
  ...
  return (
    <div
      className="rounded-xl p-3 shadow-2xl min-w-[160px]"
      style={{ backgroundColor: CARD, border: `1px solid ${BORDER}` }}
    >
      <p className="text-[10px] mb-2 font-mono" style={{ color: MUTED }}>
        {label}
      </p>
      {payload.map((entry) => (
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs" style={{ color: MUTED }}>
            {selectedPatient.bedId.toUpperCase()} · {selectedPatient.diagnosis}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-xs mt-0.5" style={{ color: MUTED }}>
                24-hour history · {selectedPatient.bedId.toUpperCase()}
              </p>
            </div>
            <div
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={trendData} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
              ...
              <Area
                type="monotone"
                dataKey={activeVital}
                name={`${activeTab.label} (${activeTab.unit})`}
                stroke={activeTab.color}
                strokeWidth={2}
                fill="url(#areaGrad)"
                dot={false}
                activeDot={{ r: 4, fill: activeTab.color, strokeWidth: 0 }}
              />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        >
          ...
          <p className="text-xs mb-5" style={{ color: MUTED }}>
            Last 12 hours · by severity
          </p>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart
              data={WARD_ALERT_FREQUENCY}
              layout="vertical"
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-red-400">
                  ICU Capacity Critical ({totalOccupied}/{totalBedsAll} Beds)
                </p>
                <p className="text-xs text-red-400/80 mt-0.5">
                  Network overflow threshold reached. Recommend initiating patient transfer
                  protocols to Metro Health Center.
                </p>
              </div>
            </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  {hasCritical && (
                    <span
                      className="text-[9px] font-bold text-red-400 px-1.5 py-0.5 rounded-md"
                      style={{
                ...
                <p className="text-[10px] mt-1.5 font-mono" style={{ color: MUTED }}>
                  {pct}% occupied
                </p>
              </div>
            );
