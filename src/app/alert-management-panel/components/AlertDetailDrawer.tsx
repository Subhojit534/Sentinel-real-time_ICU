...
import { X, Brain, User, CheckCheck, ArrowUpCircle, XCircle, MessageSquare, AlertTriangle } from 'lucide-react';
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="px-5 py-4" style={{ borderBottom: `1px solid ${BORDER}` }}>
      <p className="text-[10px] font-semibold uppercase tracking-widest mb-3 font-mono" style={{ color: MUTED }}>
        {title}
      </p>
      {children}
    </div>
    ...
    <aside className="w-96 flex-shrink-0 flex flex-col h-full overflow-hidden"
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        ...
        <Section title="Patient">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm flex-shrink-0"
              ...
              {alert.patientName.split(' ').map((n) => n[0]).join('').slice(0, 2)}
            </div>
            <div>
        ...
        <Section title="Trigger Detail">
          <div className="rounded-xl p-3.5"
            ...
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-medium" style={{ color: MUTED }}>{alert.triggerMetric}</p>
            ...
            <div className="flex items-center justify-between text-[10px]" style={{ color: MUTED }}>
              <span>Normal range: <span className="text-white font-mono">{alert.normalRange}</span></span>
        ...
        {alert.notes && (
          <Section title="Clinical Notes (SBAR)">
            <div className="rounded-xl p-3.5" style={{ backgroundColor: CARD, border: `1px solid ${BORDER}` }}>
              <p className="text-xs leading-relaxed" style={{ color: MUTED }}>{alert.notes}</p>
            </div>
          </Section>
        )}
              ...
              return (
                <div key={`esc-${step.level}`} className="flex items-start gap-3">
                  ...
                  <div className="pt-0.5">
                    <p className="text-xs font-medium" style={{ color: isReached ? 'white' : MUTED }}>
                      {step.label}
                    </p>
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] font-mono" style={{ color: MUTED }}>
                  {new Date(alert.createdAt).toLocaleTimeString('en-GB')} · {timeAgo(alert.createdAt)}
                </p>
              </div>
            </div>
            {alert.acknowledgedAt && (
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  <p className="text-xs font-medium text-white">Acknowledged by {alert.assignedTo}</p>
                  <p className="text-[10px] font-mono" style={{ color: MUTED }}>
                    {new Date(alert.acknowledgedAt).toLocaleTimeString('en-GB')} · {timeAgo(alert.acknowledgedAt)}
                  </p>
                </div>
              </div>
            )}
            {alert.resolvedAt && (
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-mono" style={{ color: MUTED }}>
                    {new Date(alert.resolvedAt).toLocaleTimeString('en-GB')} · {timeAgo(alert.resolvedAt)}
                  </p>
                </div>
              </div>
            )}
        ...
        <div className="px-5 py-4">
          <div className="flex items-center justify-between mb-2">
            <p className="text-[10px] font-semibold uppercase tracking-widest font-mono" style={{ color: MUTED }}>
              Add Note
            </p>
            <button
      ...
      <div className="p-4 flex-shrink-0 space-y-2" style={{ borderTop: `1px solid ${BORDER}` }}>
        ...
        {(alert.status === 'active' || alert.status === 'acknowledged') && (
          <button
            ...
            style={{ backgroundColor: 'rgba(139,92,246,0.10)', border: '1px solid rgba(139,92,246,0.20)' }}
          >
            <ArrowUpCircle className="w-4 h-4" />Escalate to Next Level
          </button>
        )}
        {alert.status !== 'resolved' && (
          ...
          >
            <XCircle className="w-4 h-4" />Mark Resolved
          </button>
        )}
        {alert.status === 'resolved' && (
