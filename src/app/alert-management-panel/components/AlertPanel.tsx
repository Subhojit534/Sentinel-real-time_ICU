...
export default function AlertPanel() {
  ...
  const wards = Array.from(new Set(alerts.map((a) => a.wardId))).map((id) => ({
    id,
    name: alerts.find((a) => a.wardId === id)?.wardName ?? id,
  }));
  ...
  const bulkAcknowledge = async () => {
    try {
      const updates = Array.from(selectedIds).map((id) =>
        api.alerts.update(id, { status: 'acknowledged' })
      );
  ...
  const bulkResolve = async () => {
    try {
      const updates = Array.from(selectedIds).map((id) =>
        api.alerts.update(id, { status: 'resolved' })
      );
      await Promise.all(updates);
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Page header */}
      <div
        className="px-6 pt-5 pb-4 flex-shrink-0 space-y-4"
      ...
      >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div
            className="flex items-center gap-2 rounded-xl px-3 py-1.5 flex-1 min-w-[200px] max-w-xs"
          ...
          >
            ...
            {wards.map((w) => (
              <option key={`ward-opt-${w.id}`} value={w.id}>
                {w.name}
              </option>
            ))}
          </select>
            ...
            >
              ...
              <div className="flex items-center gap-2 ml-2">
                ...
                <button
                  ...
                  className="text-xs px-2 py-1 hover:text-white transition-colors"
                  style={{ color: MUTED }}
                >
                  Clear
                </button>
                  ...
                  <td colSpan={11} className="px-6 py-16 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-xs" style={{ color: MUTED }}>
                        Try clearing the severity, status, or ward filter
                      </p>
                    </div>
                  </td>
                </tr>
                    ...
                    >
                      ...
                      <td className="px-3 py-3.5">
                        <p className="text-xs font-mono text-white">
                          {alert.bedId ? alert.bedId.toUpperCase() : '—'}
                        </p>
                      ...
                      <td className="px-3 py-3.5">
                        <p className="text-xs whitespace-nowrap" style={{ color: MUTED }}>
                          {alert.assignedTo}
                        </p>
                      </td>
                      <td className="px-3 py-3.5">
                        <div
                          className="flex items-center gap-1 text-[10px] whitespace-nowrap font-mono"
                          style={{ color: MUTED }}
                        >
                        ...
                        <div className="flex items-center gap-0.5">
                          {alert.status === 'active' && (
                            <button
                              ...
                              title="Acknowledge"
                              className="p-1.5 rounded-lg transition-all hover:bg-amber-500/10 hover:text-amber-400"
                              style={{ color: MUTED }}
                            >
                              <CheckCheck className="w-3.5 h-3.5" />
                            </button>
                          ...
                          {(alert.status === 'active' || alert.status === 'acknowledged') && (
                            <button
                              ...
                              title="Escalate"
                              className="p-1.5 rounded-lg transition-all hover:bg-violet-500/10 hover:text-violet-400"
                              style={{ color: MUTED }}
                            >
                          ...
                          {alert.status !== 'resolved' && (
                            <button
                              ...
                              title="Resolve"
                              className="p-1.5 rounded-lg transition-all hover:bg-emerald-500/10 hover:text-emerald-400"
                              style={{ color: MUTED }}
                            >
                              <XCircle className="w-3.5 h-3.5" />
                            </button>
