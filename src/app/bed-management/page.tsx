...
import {
  BedDouble, Users, AlertTriangle, CheckCircle2,
  Filter, X, UserPlus, LogOut, ChevronDown, AlertCircle,
} from 'lucide-react';
...
type BedStatus = 'occupied_critical' | 'occupied_warning' | 'occupied_stable' | 'available' | 'maintenance';
...
function AllocateModal({
  bed, wards, onClose, onSuccess,
}: { bed: any; wards: any[]; onClose: () => void; onSuccess: () => void }) {
    ...
    <Modal onClose={onClose}>
      ...
      {done ? (
        <div className="flex flex-col items-center py-6 gap-3">
          ...
          <p className="text-xs" style={{ color: MUTED }}>Bed has been assigned in MySQL.</p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: MUTED }}>Patient Name *</label>
            <input style={inp} value={name} onChange={(e) => setName(e.target.value)}
              placeholder="e.g. John Smith" required />
          </div>
          <div className="flex gap-2">
            <div className="flex-1">
              <label className="block text-xs font-medium mb-1.5" style={{ color: MUTED }}>Age</label>
              <input style={inp} type="number" min={1} max={120} value={age}
                onChange={(e) => setAge(e.target.value)} placeholder="40" />
            </div>
            <div className="flex-1">
              <label className="block text-xs font-medium mb-1.5" style={{ color: MUTED }}>Gender</label>
              <div className="relative">
                ...
                <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 pointer-events-none" style={{ color: MUTED }} />
              </div>
            </div>
          </div>
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: MUTED }}>Diagnosis *</label>
            <input style={inp} value={diagnosis} onChange={(e) => setDiag(e.target.value)}
              placeholder="e.g. Acute MI" required />
          </div>
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: MUTED }}>Initial Status</label>
            <div className="relative">
          ...
          <div className="pt-1 flex gap-3">
            <button type="button" onClick={onClose}
              ...
              style={{ backgroundColor: 'rgba(255,255,255,0.06)', color: MUTED, border: `1px solid ${BORDER}` }}>
              Cancel
            </button>
            <button type="submit" disabled={saving}
              className="flex-1 py-2 rounded-xl text-xs font-semibold transition-all"
              style={{ backgroundColor: saving ? 'rgba(59,130,246,0.4)' : 'rgba(59,130,246,0.85)', color: '#fff', border: '1px solid rgba(59,130,246,0.4)' }}>
...
/* ─── Discharge Modal ────────────────────────────────────────────────────── */
function DischargeModal({ bed, onClose, onSuccess }: { bed: any; onClose: () => void; onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!bed.patient?.id) { setErr('No patient ID found on this bed.'); return; }
    setSaving(true); setErr('');
    try {
    ...
    <Modal onClose={onClose}>
      ...
      {done ? (
        <div className="flex flex-col items-center py-6 gap-3">
          ...
          <p className="text-xs" style={{ color: MUTED }}>Removed from MySQL. Bed is now available.</p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="rounded-xl p-3 border" style={{ backgroundColor: SURFACE, borderColor: BORDER }}>
            <p className="text-xs font-semibold text-white">{bed.patient?.name ?? 'Unknown Patient'}</p>
            <p className="text-[11px] mt-0.5" style={{ color: MUTED }}>{bed.patient?.diagnosis ?? '—'}</p>
            <div className="flex gap-3 mt-2 text-[11px] font-mono" style={{ color: MUTED }}>
              <span>NEWS2: <span className="text-white font-bold">{bed.patient?.news2 ?? '—'}</span></span>
              <span>HR: <span className="text-white font-bold">{bed.patient?.vitals?.hr ?? '—'}</span></span>
            </div>
          </div>

          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: MUTED }}>Discharge Reason</label>
            <div className="relative">
          ...
          <div>
            ...
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)}
              rows={2} placeholder="Discharge summary notes…"
          ...
          <div className="pt-1 flex gap-3">
            ...
            <button type="submit" disabled={saving}
              className="flex-1 py-2 rounded-xl text-xs font-semibold transition-all"
              style={{ backgroundColor: saving ? 'rgba(239,68,68,0.4)' : 'rgba(239,68,68,0.85)', color: '#fff', border: '1px solid rgba(239,68,68,0.35)' }}>
...
export default function BedManagementPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [w, p, b] = await Promise.all([
        api.wards.list(),
        api.patients.list(),
        fetch('/api/beds').then(r => r.json()).then(d => d.beds || []),
      ]);
      setWards(w); setPatients(p); setBeds(b);
    ...
    : wards.flatMap((ward) =>
        Array.from({ length: ward.totalBeds }, (_, i) => {
          ...
          const status: BedStatus = i < ward.occupiedBeds
            ? getBedStatus(patient?.status ?? (i < ward.criticalPatients ? 'critical' : 'stable'))
            : 'available';
          return { wardId: ward.id, wardName: ward.name, bedId: `bed-${ward.id}-${i + 1}`, bedNum: i + 1, patient, status };
        })
      );
  ...
  return (
    <AppLayout>
      <div className="h-full overflow-y-auto scrollbar-thin">
        <div className="px-6 py-5 max-w-screen-2xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.backgroundColor = 'rgba(239,68,68,0.22)'; }}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { label: 'Total Beds',    value: stats.total,    icon: BedDouble,      color: '#60A5FA', bg: 'rgba(59,130,246,0.10)',  border: 'rgba(59,130,246,0.20)'  },
              { label: 'Occupied',      value: stats.occupied, icon: Users,           color: '#fbbf24', bg: 'rgba(245,158,11,0.10)',  border: 'rgba(245,158,11,0.20)'  },
              { label: 'Available',     value: stats.available,icon: CheckCircle2,    color: '#34d399', bg: 'rgba(16,185,129,0.10)',  border: 'rgba(16,185,129,0.20)'  },
              { label: 'Critical Beds', value: stats.critical, icon: AlertTriangle,   color: '#f87171', bg: 'rgba(239,68,68,0.10)',   border: 'rgba(239,68,68,0.20)'   },
            ].map((s) => {
              const Icon = s.icon;
              return (
                <div key={s.label} className="flex items-center gap-4 p-4 rounded-2xl border"
          ...
          {wards.filter((w) => wardFilter === 'all' || w.id === wardFilter).map((ward) => {
            ...
            return (
              <div key={ward.id} className="rounded-2xl border overflow-hidden"
                style={{ backgroundColor: CARD, borderColor: BORDER }}>
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {ward.criticalPatients > 0 && (
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-md text-red-400"
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-mono text-white">{ward.occupiedBeds}/{ward.totalBeds} beds</span>
                    <div className="w-32 h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: 'rgba(255,255,255,0.08)' }}>
                      <div className="h-full rounded-full transition-all"
                        style={{ width: `${pct}%`, backgroundColor: pct >= 90 ? '#EF4444' : pct >= 75 ? '#F59E0B' : '#10B981' }} />
                    </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        {isOccupied && bed.patient ? (
                          <>
                            <p className="text-xs font-semibold text-white leading-tight truncate">{bed.patient.name}</p>
                            <p className="text-[10px] mt-0.5 truncate" style={{ color: MUTED }}>{bed.patient.diagnosis}</p>
                            <div className="mt-2"><PatientStatusBadge status={bed.patient.status} /></div>
                            <div className="flex items-center justify-between mt-2 text-[10px] font-mono" style={{ color: MUTED }}>
                              <span>NEWS2: <span className="text-white font-bold">{bed.patient.news2}</span></span>
                              <span>HR: <span className="text-white font-bold">{bed.patient.vitals.hr}</span></span>
                            </div>
                            {/* Discharge hint */}
                          ...
                          <div className="flex flex-col items-center justify-center py-3">
                            ...
                            <p className="text-[10px] font-semibold" style={{ color: style.border }}>{style.label}</p>
                            {/* Allocate hint */}
          ...
          <div className="flex items-center gap-6 pb-2">
            {Object.entries(bedColor).filter(([k]) => k !== 'maintenance').map(([key, val]) => (
              <div key={key} className="flex items-center gap-2 text-[11px]" style={{ color: MUTED }}>
      ...
      {/* Modals */}
      {allocateBed  && <AllocateModal  bed={allocateBed}  wards={wards} onClose={() => setAllocateBed(null)}  onSuccess={fetchData} />}
      {dischargeBed && <DischargeModal bed={dischargeBed}              onClose={() => setDischargeBed(null)} onSuccess={fetchData} />}
    </AppLayout>
  );
}
