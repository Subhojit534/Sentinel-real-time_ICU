...
function getBedStatus(patientStatus?: string): BedStatus {
  if (!patientStatus) return 'available';
  if (patientStatus === 'critical' || patientStatus === 'code') return 'occupied_critical';
  if (patientStatus === 'warning') return 'occupied_warning';
...
export default function BedManagementPage() {
  ...
  const allBeds = wards.flatMap((ward) =>
    Array.from({ length: ward.totalBeds }, (_, i) => {
      ...
      return { wardId: ward.id, wardName: ward.name, bedId: bedId || `bed-${ward.id}-${i+1}`, bedNum: i + 1, patient, status };
    })
  );
              ...
              <div key={ward.id} className="rounded-2xl border overflow-hidden"
                ...
                <div className="p-5 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                  {wardBeds.map((bed) => {
                    ...
                    return (
                      <div key={`${bed.wardId}-${bed.bedNum}`}
                        className="rounded-xl p-3 border transition-all duration-150"
                        style={{
                        ...
                        {isOccupied && bed.patient ? (
                          ...
                          <div className="flex flex-col items-center justify-center py-3">
                            ...
                            <p className="text-[10px] font-semibold" style={{ color: style.border }}>
                              {style.label}
                            </p>
                          </div>
                        )}
