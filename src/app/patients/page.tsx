        ...
        <div className="px-5 py-4 space-y-3" style={{ borderBottom: `1px solid ${BORDER}` }}>
          ...
          {[
            ...
            { icon: Calendar, label: 'Admitted', value: patient.admissionDate },
            {
              icon: User,
              label: 'Bed',
              value: `${patient.bedId.toUpperCase()} · ${patient.wardId.replace('ward-icu-', 'ICU ').toUpperCase()}`,
            },
            ...
            {vitals.map(({ label, value, alert, color }) => (
              ...
              >
                ...
                <p
                  ...
                  style={{ color: alert ? color : 'white' }}
                >
                  {value}
                </p>
                {alert && (
                  <p className="text-[9px] mt-0.5 font-semibold" style={{ color }}>
                    ABNORMAL
                  </p>
                )}
              </div>
        ...
        <div className="flex-1 min-w-0 overflow-y-auto scrollbar-thin">
          <div className="px-6 py-5 space-y-5 max-w-screen-2xl mx-auto">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  id="btn-export-pdf"
                  onClick={() =>
                    checkedIds.size > 0 ? handleExport(selectedPatients) : handleExport(sorted)
                  }
                  disabled={exporting}
                  className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all active:scale-[0.97]"
                  style={{
                    backgroundColor:
                      checkedIds.size > 0 ? 'rgba(59,130,246,0.85)' : 'rgba(59,130,246,0.15)',
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <div
                className="flex items-center gap-2 rounded-xl px-3 py-1.5 flex-1 min-w-[240px] max-w-sm"
                style={{ backgroundColor: CARD, border: `1px solid ${BORDER}` }}
              >
                <Search className="w-3.5 h-3.5 flex-shrink-0" style={{ color: MUTED }} />
                <input
                  type="text"
                  placeholder="Search name, diagnosis, bed, physician..."
                  value={search}
                    ...
                    >
                      {[
                        ...
                        },
                        {
                          ...
                          unit: 'mmHg',
                          alert: patient.vitals.map < 65,
                        },
                      ].map(({ icon: Icon, label, value, unit, alert }) => (
                        <div key={label} className="text-center">
                          <p
                            className="text-[9px] uppercase font-medium mb-0.5"
                            style={{ color: MUTED }}
                          >
                            {label}
                          </p>
