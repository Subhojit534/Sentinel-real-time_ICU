...
export default function AuthBrandPanel() {
    ...
    useEffect(() => {
        ...
        let scene: THREE.Scene, camera: THREE.PerspectiveCamera, renderer: THREE.WebGLRenderer, mainGroup: THREE.Group, pivot: THREE.Object3D;
        let isSolving = false;
        ...
        function createRubiksCube() {
            ...
            const getFaceMaterial = (colorCode: number) => new THREE.MeshPhysicalMaterial({
                color: colorCode,
                emissive: colorCode,
    ...
    return (
        <div className="hidden lg:flex lg:w-[52%] xl:w-[55%] flex-col relative overflow-hidden landing-body border-l border-border">
            ...
            {/* Rubik's Cube Canvas */}
            <div id="canvas-container" ref={canvasContainerRef} className="absolute inset-0 z-10 w-full h-full"></div>

            {/* Project Sentinel Branding overlay */}
            <div className="absolute top-10 left-10 z-20 flex items-center gap-3">
                <AppLogo size={40} />
                <div>
                    <span className="text-lg font-bold text-white tracking-tight block leading-none drop-shadow-md">ProjectSentinel</span>
                    <span className="text-[11px] text-cyan-300 tracking-widest uppercase drop-shadow-sm mt-1">ICU Monitoring Platform</span>
                </div>
            </div>

            {/* Interactive feedback overlay */}
            <div id="solve-status" className="absolute text-xs font-bold tracking-widest transition-opacity duration-300 opacity-0 pointer-events-none bottom-10 left-10 text-cyan-300 font-mono z-20 drop-shadow-md">
                SOLVING ALGORITHM...
            </div>
